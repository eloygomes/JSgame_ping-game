...
let scores, currentStore, activePlayer, playing;
...
btnRoll.addEventListener('click', function () {
  if (playing) {
    // 1. Gerenating a random number
    const dice = Math.trunc(Math.random() * 6) + 1;
    // 2. Display dice
    diceEl.classList.remove('hidden');
    diceEl.src = `dice-${dice}.png`;

    // 3. Checked for rolled in 1:
    if (dice !== 1) {
      //Add dice to current store
      currentStore += dice; // This is the same of currentStore = currentStore + dice
